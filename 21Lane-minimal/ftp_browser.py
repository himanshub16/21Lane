...
class FTPBrowser(QWidget):
	...
	def initUI(self):
		...
		self.searchContainer = QGroupBox("")
		self.searchContainer.setTitle("Show | Search Box")
		self.searchContainer.setCheckable(True)
		self.searchContainer.clicked.connect(self.toggleSearchContainer)
	...
	def toggleSearchContainer(self):
		if (self.searchContainer.isChecked()):
			self.searchContainer.setTitle("Show | Search Box")
			self.searchCategoryBox.hide()
			self.actionsGroupBox.hide()
		else:
			self.searchCategoryBox.show()
			self.searchContainer.setTitle("Hide | Search Box")
			self.actionsGroupBox.show()
			self.searchCategoryBox.setEnabled(True)
			self.actionsGroupBox.setEnabled(True)
	...
	def downloadDirectory(self, dirpath):
		def callDownloadClient():
			...
			for file in filelist: 
				...
				self.downloadFile(filename=file['filename'], pathname=file['pathname'], filesize=file['filesize'], destpath=file['pathname'])()
